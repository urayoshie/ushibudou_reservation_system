class DayCondition < ApplicationRecord
  ...
  def prevent_earlier_date
    if DayCondition.count.positive? && applicable_date < DayCondition.initial_date
  ...
  class << self
    def initial_date
      DayCondition.order(applicable_date: :asc).first&.applicable_date
    end

    # start_date から end_date までの規定営業日の配列を取得
    def default_business_dates(start_date, end_date)
    ...
    def default_closed_dates(start_date, end_date)
      # 休業日設定の入っている曜日の配列
